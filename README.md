<div align="center">

<img src="https://raw.githubusercontent.com/Arnoldzzz21/Arnoldzzz21/main/assets/banner.svg" width="100%"/>

</div>

"Hi! I'm a curious person with a background in Economics who discovered a passion for Data Science and AI.

I've worked with data in BI and Data Science, turning complex information into useful dashboards, models, and insights.

I'm currently growing my skills in Artificial Intelligence and always enjoy solving problems with data."

<div align="center">

![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![Pandas](https://img.shields.io/badge/Pandas-150458?style=for-the-badge&logo=pandas&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=for-the-badge&logo=postgresql&logoColor=white)
![Jupyter](https://img.shields.io/badge/Jupyter-F37626?style=for-the-badge&logo=jupyter&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)

</div>

### My Projects

> [!NOTE]
> | Project | Description | Libraries | Model | Result |
> |---|---|---|---|---|
> | [Superstore Sales & Profitability Analysis](https://github.com/Arnoldzzz21/Superstore-Project) | Retail sales and profitability analysis: EDA, customer segmentation, and the factors that affect margin. | Pandas, NumPy, Matplotlib, Seaborn, Scikit-learn | K-Means Clustering, Linear Regression | [View dashboard](https://arnoldzzz21.github.io/Superstore-Project/dashboardsuperstore.html) |
> | [Motorcycle Price Prediction](https://github.com/Arnoldzzz21/motorcycle-price-prediction) | Predicting used motorcycle resale price from brand, category, year, condition, and mileage. | Pandas, NumPy, Matplotlib, Seaborn, Scikit-learn, XGBoost | Linear Regression, Random Forest, Gradient Boosting, XGBoost (tuned) | [View ML App](https://arnoldzzz21-motorcycle-price-prediction.streamlit.app/) |
> | [CoinVision](https://github.com/Arnoldzzz21/coinvision-app) | Identifies the country and denomination of a coin from a photo, with Grad-CAM explainability. | PyTorch, Torchvision, OpenCV, Streamlit | EfficientNet-B3 (transfer learning + fine-tuning) | [View App](https://coinvision-arnoldo.streamlit.app/) |
